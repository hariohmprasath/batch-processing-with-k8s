...
// Main class
export class KubernetesFileBatchConstruct extends cdk.Construct {
  readonly vpc: ec2.IVpc;
  readonly minNodes?: number;
  readonly maxNodes?: number;
  readonly desiredNodes?: number;
  ...
  constructor(scope: cdk.Construct, id: string, props: KubernetesFileBatchInput) {
    super(scope, id);
    ...
    const redisSubnetGroup = new elasticcache.CfnSubnetGroup(this, this.getId('RedisClusterPrivateSubnetGroup'), {
      subnetIds: this.vpc.selectSubnets({ subnetType: ec2.SubnetType.PRIVATE }).subnetIds,
    ...
    const stepFunctionRole = this.getRole('stateRole', 'states.us-east-1.amazonaws.com',
  ...
  setupEfsArtifacts(cluster: eks.Cluster, securityGroup: ec2.SecurityGroup): efs.FileSystem {
    ...
    const efsFileSystem = new efs.FileSystem(this, this.getId('FileSystem'), {
      ...
      vpcSubnets: this.vpc.selectSubnets({ subnetType: ec2.SubnetType.PRIVATE }),
    });
